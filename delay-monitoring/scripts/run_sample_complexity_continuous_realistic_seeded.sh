...
#
# Realistic-mode continuous experiment with 100 random seeds.
...
NS3_DIR="ns-3.46"
RESULTS_DIR="delay-monitoring/results/sample-complexity/continuous/realistic/seeded"
RESULTS_RELPATH="../delay-monitoring/results/sample-complexity/continuous/realistic/seeded"
QUEUE_SIZE=50
...
total=$(( ${#DISTRIBUTIONS[@]} * ${#SAMPLE_SIZES[@]} * ${#SEEDS[@]} ))
count=0

for dist in "${DISTRIBUTIONS[@]}"; do
    echo "=== ${dist} — ${#SEEDS[@]} seeds ==="
    for seed in "${SEEDS[@]}"; do
        for n in "${SAMPLE_SIZES[@]}"; do
            count=$(( count + 1 ))
            output="${RESULTS_RELPATH}/delay_samples_${dist}_${n}_seed${seed}.csv"

            if [ -f "$output" ]; then
                echo "  [${count}/${total}] skip (exists): ${dist} n=${n} seed=${seed}"
                continue
            fi
