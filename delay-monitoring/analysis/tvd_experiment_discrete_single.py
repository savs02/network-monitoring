"""
TVD experiment — discrete distributions (single seed).

Reads the CSVs generated by run_sample_complexity_discrete.sh.
Since there is only one run per sample size, results are shown as a
line plot of TVD vs n rather than a box plot.
...
RESULTS_DIR = os.path.join(SCRIPT_DIR, "../results/sample-complexity-discrete")
...
def plot_distribution(emp_pmf, true_pmf_vals, support, dist_config, n, tvd_val):
    """Bar chart of empirical PMF vs true PMF for one sample count."""
...
def plot_tvd_vs_n(results):
    """
    Line plot of TVD vs n for both distributions on the same axes,
    with theoretical bounds overlaid.
    """
